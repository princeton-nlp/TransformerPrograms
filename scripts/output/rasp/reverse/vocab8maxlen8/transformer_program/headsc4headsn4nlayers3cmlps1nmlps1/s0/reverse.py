...
def run(tokens):
    ...
    classifier_weights = pd.read_csv(
        "output/rasp/reverse/vocab8maxlen8/transformer_program/headsc4headsn4nlayers3cmlps1nmlps1/s0/reverse_weights.csv",
        index_col=[0, 1],
        dtype={"feature": str},
    )
    # inputs #####################################################
    ...
    # num_attn_1_2 ####################################################
    def num_predicate_1_2(position, num_mlp_0_0_output):
        if position in {0, 1, 3, 4, 5, 6, 7}:
            return num_mlp_0_0_output == 3
        elif position in {2}:
            return num_mlp_0_0_output == 2

    num_attn_1_2_pattern = select(num_mlp_0_0_outputs, positions, num_predicate_1_2)
    ...
    # mlp_1_0 #####################################################
    def mlp_1_0(attn_0_0_output, attn_1_0_output):
        ...
        if key in {
            ...
            return 4
        elif key in {("1", "</s>"), ("2", "</s>"), ("3", "</s>")}:
            return 3
        return 6

    mlp_1_0_outputs = [
        mlp_1_0(k0, k1) for k0, k1 in zip(attn_0_0_outputs, attn_1_0_outputs)
    ]
    ...
    attn_2_3_outputs = aggregate(attn_2_3_pattern, num_mlp_1_0_outputs)
    attn_2_3_output_scores = classifier_weights.loc[
        [("attn_2_3_outputs", str(v)) for v in attn_2_3_outputs]
    ]

    # num_attn_2_0 ####################################################
    def num_predicate_2_0(attn_1_0_output, num_mlp_1_0_output):
        if attn_1_0_output in {"0", "1"}:
            return num_mlp_1_0_output == 3
        elif attn_1_0_output in {"2", "<s>"}:
            return num_mlp_1_0_output == 5
        elif attn_1_0_output in {"3", "4"}:
            return num_mlp_1_0_output == 6
        elif attn_1_0_output in {"</s>"}:
            return num_mlp_1_0_output == 7

    num_attn_2_0_pattern = select(
        num_mlp_1_0_outputs, attn_1_0_outputs, num_predicate_2_0
    )
    ...
    # num_attn_2_1 ####################################################
    def num_predicate_2_1(attn_1_0_output, num_mlp_0_0_output):
    ...
    num_attn_2_1_pattern = select(
        num_mlp_0_0_outputs, attn_1_0_outputs, num_predicate_2_1
    )
    ...
    # num_attn_2_3 ####################################################
    def num_predicate_2_3(attn_1_0_output, num_mlp_0_0_output):
        if attn_1_0_output in {"3", "4", "0"}:
            return num_mlp_0_0_output == 3
        elif attn_1_0_output in {"1"}:
            return num_mlp_0_0_output == 2
    ...
    num_attn_2_3_pattern = select(
        num_mlp_0_0_outputs, attn_1_0_outputs, num_predicate_2_3
    )
